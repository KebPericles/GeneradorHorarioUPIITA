...
<svelte:window
	onkeydown={(e) => {
		...
		e.preventDefault();
		document.getElementById('vista-horario')?.scrollIntoView({ behavior: 'smooth' });
...
<nav class="flex h-10 w-full justify-center gap-2">
	<button
		class="flex h-full w-auto justify-center"
		disabled={pagina === 0}
	...
	<button
		class="has-tooltip flex h-full w-auto justify-center"
		disabled={pagina === 0}
		aria-label="Anterior"
	...
	<div class="tooltip relative h-0 w-0">
		<div
			class="relative -top-2 z-50 w-36 rounded-lg bg-black px-3 py-1.5 font-sans text-sm font-normal break-words whitespace-normal text-white focus:outline-none"
		>
			Puedes usar la flecha izquierda.
		</div>
	</div>
	<div class="flex w-md justify-center self-center align-middle text-xl">
		{#if paginas > 0}
			{pagina + 1} / {paginas}
		{:else}
			0
	...
	<div class="tooltip relative -left-19 h-0 w-0">
		<div
			class="relative -top-2 -left-36 z-50 w-36 rounded-lg bg-black px-3 py-1.5 font-sans text-sm font-normal break-words whitespace-normal text-white focus:outline-none"
		>
